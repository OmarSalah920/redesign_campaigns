...
interface FormData {
  ...
  concurrency: number;
  concurrencyType: 'manual' | 'automatic';
  groupName: string;
}
...
const AGENT_GROUPS = [
  { id: 'sales', name: 'Sales Team', agentCount: 12, onlineCount: 8 },
  { id: 'support', name: 'Support Team', agentCount: 15, onlineCount: 10 },
  { id: 'marketing', name: 'Marketing Team', agentCount: 8, onlineCount: 5 },
  { id: 'general', name: 'General Agents', agentCount: 20, onlineCount: 14 }
];
...
export const NewCampaignModal: React.FC<NewCampaignModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.concurrencyType === 'manual' && (formData.concurrency < 1 || formData.concurrency > 100)) {
      newErrors.concurrency = 'Concurrency must be between 1 and 100';
    }

    if (formData.concurrencyType === 'automatic' && !formData.groupName) {
      newErrors.groupName = 'Agent group selection is required for automatic concurrency';
    }
  ...
  const selectedGroup = AGENT_GROUPS.find(group => group.id === formData.groupName);
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-140px)]">
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            <div className="space-y-6">
              ...
              {/* Row 1: IVR (left) and Maximum Tries (right) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* IVR Selection */}
                <div>
              ...
              {/* Row 2: Retry Interval (full width) */}
              <div>
                <TimePicker
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-6">
                    <label className="flex items-center">
                      <input
                        ...
                        value="manual"
                        checked={formData.concurrencyType === 'manual'}
                        onChange={(e) => setFormData(prev => ({ ...prev, concurrencyType: e.target.value as 'manual' | 'automatic' }))}
                        className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                      />
                      <span className="ml-2 text-sm font-medium text-gray-700">Manual Concurrency</span>
                    </label>
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        name="concurrencyType"
                        value="automatic"
                        checked={formData.concurrencyType === 'automatic'}
                        onChange={(e) => setFormData(prev => ({ ...prev, concurrencyType: e.target.value as 'manual' | 'automatic' }))}
                        className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                      />
                      <span className="ml-2 text-sm font-medium text-gray-700">Automatic Concurrency</span>
                    </label>
                  </div>

                  {/* Manual Concurrency Input */}
                  {formData.concurrencyType === 'manual' && (
                    <div>
                      ...
                      <input
                        ...
                        className="form-input h-12 max-w-xs"
                  ...
                  {/* Automatic Concurrency - Group Selection */}
                  {formData.concurrencyType === 'automatic' && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        <div className="flex items-center">
                              ...
                              {selectedGroup ? (
                                <div className="flex items-center justify-between w-full">
                                  ...
                                  <span className="text-sm text-gray-500 ml-2">
                                    {selectedGroup.onlineCount}/{selectedGroup.agentCount} online
                                  </span>
                                </div>
                              ) : (
                                'Select agent group'
                            ...
                            {AGENT_GROUPS.map((group, index) => (
                              <button
                                ...
                                className={`w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors duration-200 ${
                                  index !== AGENT_GROUPS.length - 1 ? 'border-b border-gray-100' : ''
                                } ${
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Concurrency will automatically adjust based on online agents in this group
                      </p>
                    </div>
                  )}
                </div>
